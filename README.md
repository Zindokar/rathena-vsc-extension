...
- **Map-server parser** — a faithful port of rAthena's own `parse_script`, reporting the exact error the server would print, in the exact format it would print it.
...
|---|---|---|---|
| <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>C</kbd> | <kbd>Cmd</kbd>+<kbd>Alt</kbd>+<kbd>C</kbd> | rAthena: Check With Map-Server Parser | Parses the current file with the ported parser. Instant |
| <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>M</kbd> | <kbd>Cmd</kbd>+<kbd>Alt</kbd>+<kbd>M</kbd> | rAthena: Run Real Map-Server Script Check | Spawns your compiled binary. Slow, optional |

<kbd>Ctrl/Cmd</kbd>+<kbd>Alt</kbd>+<kbd>C</kbd> is the one you will use. It looks only at the file you have open, needs nothing installed, and its notification carries the file and line with a **Go to error** button.
...
- **Check With Map-Server Parser** — parse the current file
- **Run Real Map-Server Script Check** — run the compiled binary
...
**rAthena: Run Real Map-Server Script Check** (<kbd>Ctrl/Cmd</kbd>+<kbd>Alt</kbd>+<kbd>M</kbd>) spawns your compiled binary with `--run-once`, which loads everything and exits. Its console output is streamed to an output channel and its `script error on <file> line <n>` blocks become diagnostics on the right lines.

It exists because a parser — ours or a perfect one — only ever sees one file. Duplicate NPC names across files, warps pointing at a map missing from the map cache: those only surface when the whole server loads. It needs a built server (`./configure && make sql`) and a reachable database, so it is entirely optional; the first two tiers need nothing but the extension.

Configure it with `rathena.mapServer.binary` and `rathena.mapServer.args`.
...
| `rathena.mapServer.binary` | *(auto-detect)* | Path to the compiled `map-server` |
| `rathena.mapServer.args` | `["--run-once"]` | Arguments passed to it |
